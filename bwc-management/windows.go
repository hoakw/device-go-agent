...
func main() {
	...
	// Set Service Variable
	winSvcInfo := winManagementService{
		MqttType: mqttType,
		ArchType: archType,
		RootPath: rootPath,
	}
	err = winSvc.Run("BWCManagementService", &winSvcInfo)
	if err != nil {
...
func (srv *winManagementService) Execute(args []string, req <-chan winSvc.ChangeRequest, stat chan<- winSvc.Status) (svcSpecificEC bool, exitCode uint32) {
	stat <- winSvc.Status{State: winSvc.StartPending}

	// 실제 서비스 내용
	procLog.Info.Printf("[SVC] Service Content!!!\n")
	stopChan := make(chan bool, 1)
	go sdtManagement.RunBody(srv.MqttType, srv.ArchType, srv.RootPath)
