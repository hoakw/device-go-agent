...
func (srv *healthService) Execute(args []string, req <-chan winSvc.ChangeRequest, stat chan<- winSvc.Status) (svcSpecificEC bool, exitCode uint32) {
...
func main() {
	...
	err = winSvc.Run("DeviceHealthService", &svcInfo)
	if err != nil {
		panic(err)
	}
}
