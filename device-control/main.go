...
func SubMessage(client mqttCli.Client, msg mqttCli.Message) {
	...
	if err != nil {
		...
		procLog.Info.Printf("[MQTT] Send ACK message. %s\n", ackMsg)
...
func RunBody() {
	...
	//		"heartbeat":       BWC_MANAGEMENT_VERSION,
	//	},
	//}
	//
	//if configData.DeviceType == "aquarack" {
	//	procLog.Info.Printf("[SUJUNE] Version Info: %s\n", runtimeResult)
	//	bwcVersionMap := runtimeResult["bwcVersion"].(map[string]interface{})
	//	bwcVersionMap["aquarack"] = AQUARACK_VERSION
	//}
	//
	//procLog.Info.Printf("[SUJUNE] Version Info: %s\n", runtimeResult)
	//
